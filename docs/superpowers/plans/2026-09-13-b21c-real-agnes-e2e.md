...
本计划在 Phase A 只执行任务 1，然后 STOP。用户随后已明确授权一次 Create（本地记录时间 `2026-09-13T14:25:42Z`），当前按 Phase B 执行任务 2–6。它们先使用模拟 transport 通过全部检查，再执行独立的 REAL CREATE 步骤。密钥配置本身不构成授权；未勾选的实施与验收步骤仍待完成。

实施拆分：任务 3a 负责 Adapter 安全字段，3b 负责持久化来源与实际执行路由，3c 只修正真实生成结果的预览来源标记。任务 4 负责原 HTTP 入口下的单 Item 策略和受控会话；共享常量与类型通过已审查接口衔接。永久预算采用 Git common directory，业务数据保持在原启动器允许的 worktree 内。
